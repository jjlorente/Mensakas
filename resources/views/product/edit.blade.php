...
@section('content')
			...
			<form method="POST" action="{{ route('product.update',$product->product_id) }}"  role="form" style="width: 50%;align-content: center;margin: 0 auto;">
				{{ csrf_field() }}
				<input name="_method" type="hidden" value="PATCH">
					...
					<div class="form-group" style="display: flex; flex-direction: row;justify-content: space-between; ">
						...
						<input type="text" name="description" id="description" class="form-control input-sm" value="{{$product->description}}">
					</div>
				
					<div class="form-group" style="display: flex; flex-direction: row;">
						<h6 style="width: 10%;margin-top: 10px;">Status: </h6>
						<input type="hidden" value="0" name="status">
         				<input type="checkbox" value="1" name="status" style="position: relative; margin-left: 0;" @if($product->status) checked @endif>
					</div>					
